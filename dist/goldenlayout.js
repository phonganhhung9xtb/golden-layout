(function ($) {
	...
	lm.utils.DragListener = function (eElement, nButtonCode) {
		 ...
		 */
		var browerInfo = getBrowser() || { name: '', version: '' };
		console.log('vBrowerInfo: ', browerInfo)
		this._nDelay = browerInfo.name.toLocaleLowerCase() === 'safari' ? 1000 : 500;
	...
	lm.utils.copy(lm.items.Stack.prototype, {
		...
		_$init: function () {
			...
			if (this.contentItems.length > 0) {
				...
				if (!initialItem) {
					console.warn('Configured activeItemIndex out of bounds before set default');
					initialItem = this.contentItems[0];
				}
				if (!initialItem) {
